...
- **✨ v0.8.70 — In-app update notifier + correctness fixes**
  ...
  - **📝 Deferred:** the four locales `ca-ES`/`de-DE`/`pl-PL`/`tr-TR` are missing large parts of the translation set (not just `studio.*`), so the frontend locale-parity test was already red before this change; a full translation pass is tracked separately. All new update-notifier UI uses inline `defaultValue` strings (the existing banner pattern), so it adds no new locale-parity gaps.
