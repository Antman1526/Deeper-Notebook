...
- **✨ v0.8.68 — Offline/online smart switching + Offline-mode toggle**
  ...
  - **✨ Welcome splash:** the main window now opens on a polished inline welcome page (animated gradient, glass card, rotating status, "everything runs on your Mac" footer) that paints instantly with zero network dependencies, probes the frontend with no-cors fetches, and fades into the app once the server answers twice in a row. There is no terminal error state — a slow frontend gets "taking a little longer…" and unlimited retries instead of WKWebView's dead error page. The loaded-event handler only stands the retry watchdog down for real frontend pages, so the splash itself can't disarm the last-resort recovery (`desktop/splash.py`, `desktop/tests/test_v0_8_68_welcome_splash.py`).
