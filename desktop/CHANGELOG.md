...
- **✨ v0.8.72 — Premium theme pack, theme-aware aurora visuals & cold-start fix**
  ...
  - **🐛 BUG FIX — cold-start flicker to a "reload page":** `ConnectionGuard`'s startup `/api/config` check retried for only ~6s, but a cold desktop boot is ~30s (and up to ~2 min on a first launch where the model scan stalls). The retry exhausted before the backend was ready, so the app **flickered to the full-screen "Unable to connect — Retry" overlay**, and only a manual reload — by which point the API was finally up — recovered it. The check now retries against a **120s time budget** while showing a quiet "Connecting…", and a **background poll self-heals** the overlay if it ever does appear, so no manual reload is ever needed. (Supersedes the ~6s backoff added in v0.8.71.)
