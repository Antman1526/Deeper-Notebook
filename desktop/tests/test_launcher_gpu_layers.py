...
def test_macos_default_offloads_all_layers(monkeypatch):
    monkeypatch.delenv("ONP_CHAT_LLM_N_GPU_LAYERS", raising=False)
    monkeypatch.setattr("desktop.launcher.sys.platform", "darwin")
    assert _n_gpu_layers("ONP_CHAT_LLM_N_GPU_LAYERS") == "-1"


def test_non_macos_defaults_to_cpu(monkeypatch):
    monkeypatch.delenv("ONP_CHAT_LLM_N_GPU_LAYERS", raising=False)
    monkeypatch.setattr("desktop.launcher.sys.platform", "linux")
    assert _n_gpu_layers("ONP_CHAT_LLM_N_GPU_LAYERS") == "0"


def test_env_override_is_honored(monkeypatch):
    monkeypatch.setattr("desktop.launcher.sys.platform", "darwin")
    monkeypatch.setenv("ONP_CHAT_LLM_N_GPU_LAYERS", "20")
    assert _n_gpu_layers("ONP_CHAT_LLM_N_GPU_LAYERS") == "20"


def test_env_override_can_force_cpu_on_mac(monkeypatch):
    monkeypatch.setattr("desktop.launcher.sys.platform", "darwin")
    monkeypatch.setenv("ONP_CHAT_LLM_N_GPU_LAYERS", "0")
    assert _n_gpu_layers("ONP_CHAT_LLM_N_GPU_LAYERS") == "0"
...
def test_unparseable_env_falls_back_to_mac_default(monkeypatch, bad):
    ...
    monkeypatch.setattr("desktop.launcher.sys.platform", "darwin")
    monkeypatch.setenv("ONP_CHAT_LLM_N_GPU_LAYERS", bad)
    assert _n_gpu_layers("ONP_CHAT_LLM_N_GPU_LAYERS") == "-1"


def test_custom_mac_default(monkeypatch):
    monkeypatch.delenv("ONP_EMBED_N_GPU_LAYERS", raising=False)
    monkeypatch.setattr("desktop.launcher.sys.platform", "darwin")
    assert _n_gpu_layers("ONP_EMBED_N_GPU_LAYERS", mac_default=0) == "0"
