...
def test_clamp_caps_to_screen():
    ...
    assert window_state.clamp(3000, 2000, 1440, 900, min_w=1280, min_h=800) == (1440, 900)


def test_clamp_keeps_value_in_range():
    assert window_state.clamp(1600, 1000, 1920, 1200, min_w=1280, min_h=800) == (1600, 1000)
...
def test_clamp_bad_input_returns_floor():
    assert window_state.clamp("x", None, 1920, 1200, min_w=1280, min_h=800) == (1280, 800)
