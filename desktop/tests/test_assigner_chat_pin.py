"""v0.8.67h — ONP_CHAT_LLM_GGUF pins a specific chat model over the heuristic.
...
model choice. ONP_CHAT_LLM_GGUF lets the user force a specific file; if unset or
...
def test_pin_forces_named_model(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ONP_CHAT_LLM_GGUF", "Qwen3.5-9B-Q4_K_M.gguf")
...
def test_pin_without_extension_and_case_insensitive(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ONP_CHAT_LLM_GGUF", "qwen3.5-9b-q4_k_m")  # no .gguf, lowercase
    assert pick_chat_llm_file(tmp_path).name == "Qwen3.5-9B-Q4_K_M.gguf"


def test_pin_not_found_falls_through_to_heuristic(tmp_path, monkeypatch):
    _mk(tmp_path, ["Hermes-3-Llama-3.1-8B-Q4_K_M.gguf"])
    monkeypatch.setenv("ONP_CHAT_LLM_GGUF", "DoesNotExist.gguf")
...
def test_no_pin_uses_heuristic(tmp_path, monkeypatch):
    monkeypatch.delenv("ONP_CHAT_LLM_GGUF", raising=False)
    _mk(tmp_path, ["Hermes-3-Llama-3.1-8B-Q4_K_M.gguf"])
    r = pick_chat_llm_file(tmp_path)
