...
def _build_llm_wrapper(base_url: str = "http://localhost:8080/v1",
                      model: str = "default",
                      timeout_s: float = 5.0):
...
def test_complete_returns_empty_on_http_503():
    ...
    with patch("httpx.Client.post",
               return_value=_fake_response(503, {"error": "loading"})):
        llm = _build_llm_wrapper()
...
def test_complete_returns_empty_on_error_response_with_200():
    ...
    with patch("httpx.Client.post",
               return_value=_fake_response(200, error_payload)):
        llm = _build_llm_wrapper()
        result = llm.complete("sys", "user")
    assert result == ""
...
def test_complete_returns_empty_on_empty_choices_array():
    ...
    with patch("httpx.Client.post",
               return_value=_fake_response(200, {"choices": []})):
        llm = _build_llm_wrapper()
        result = llm.complete("sys", "user")
    assert result == ""
