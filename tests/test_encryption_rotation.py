...
`OPEN_NOTEBOOK_ENCRYPTION_KEY`. Rotating the key invalidated every
stored credential — for a local user that means re-entering every
API key for every provider after rotation, a hostile UX.

v0.7.17 adds `OPEN_NOTEBOOK_ENCRYPTION_KEYS` (plural, comma-separated)
...
def test_singular_env_var_still_works(monkeypatch):
    ...
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", raising=False)
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", "old-passphrase")
...
def test_plural_env_var_takes_precedence(monkeypatch):
    ...
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "new-key,old-key")
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", "ignored")
...
def test_no_keys_configured_raises(monkeypatch):
    ...
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS_FILE", raising=False)
...
def test_empty_plural_falls_back_to_singular(monkeypatch):
    """An empty `OPEN_NOTEBOOK_ENCRYPTION_KEYS=` (e.g. accidentally
    overridden in CI) should NOT mask the singular fallback."""
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "  ,  ,  ")
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", "fallback")
...
def test_encrypt_decrypt_single_key(monkeypatch):
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", "single-key")
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", raising=False)
...
def test_data_encrypted_under_old_key_decrypts_after_adding_new(monkeypatch):
    ...
    encryption._reset_encryption_cache()
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "new-secret")
...
def test_data_encrypted_under_old_key_fails_when_old_dropped_prematurely(monkeypatch):
    ...
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", "doomed-old")
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", raising=False)
    old_cipher = encryption.encrypt_value("important-creds")
    encryption._reset_encryption_cache()

    # User mistakenly removes the old key.
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "shiny-new")
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", raising=False)
    ...
    assert "OPEN_NOTEBOOK_ENCRYPTION_KEYS" in str(exc_info.value)
...
def test_re_encrypt_value_rotates_under_primary_key(monkeypatch):
    ...
    encryption._reset_encryption_cache()
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "new-key")
    assert encryption.decrypt_value(rotated_cipher) == "my-secret-credential"
...
def test_re_encrypt_with_unknown_key_raises(monkeypatch):
    ...
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", "mystery-key")
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", raising=False)
    orphan_cipher = encryption.encrypt_value("lost-forever")
    encryption._reset_encryption_cache()

    # Switch to a config that doesn't include mystery-key
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "new-key,older-key")
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", raising=False)
...
def test_get_fernet_returns_primary_key(monkeypatch):
    ...
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "primary,secondary")
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", raising=False)
...
def test_legacy_plaintext_decrypt_passthrough(monkeypatch):
    ...
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "k1,k2,k3")
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", raising=False)
...
def test_re_encrypt_is_idempotent_under_same_primary(monkeypatch):
    ...
    monkeypatch.setenv("OPEN_NOTEBOOK_ENCRYPTION_KEYS", "alpha,beta")
    monkeypatch.delenv("OPEN_NOTEBOOK_ENCRYPTION_KEY", raising=False)
