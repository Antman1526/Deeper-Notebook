...
class TestComposite:
    def test_all_pass_fast_is_max_score(self):
        ...
        rep = ModelReport(
            model_id="m:1", model_name="m1", provider="p", model_type="language",
        )
        for probe in ("NOTEBOOK_CHAT", "STUDIO_OUTLINE", "PODCAST_TRANSCRIPT_TURN"):
            rep.results.append(
                ProbeResult(
                    probe=probe, model_id="m:1", model_name="m1", provider="p",
                    success=True, elapsed_s=0.3,
                )
            )
    ...
    def test_all_fail_slow_is_min_score(self):
        from scripts.benchmark_models import ModelReport, ProbeResult
        rep = ModelReport(
            model_id="m:2", model_name="m2", provider="p", model_type="language",
        )
        for probe in ("NOTEBOOK_CHAT", "STUDIO_OUTLINE", "PODCAST_TRANSCRIPT_TURN"):
            rep.results.append(
                ProbeResult(
                    probe=probe, model_id="m:2", model_name="m2", provider="p",
                    success=False, elapsed_s=90.0,
                )
            )
    ...
    def test_pass_dominates_over_latency(self):
        ...
        slow_pass = ModelReport(
            model_id="m:3", model_name="m3", provider="p", model_type="language",
        )
        for probe in ("NOTEBOOK_CHAT", "STUDIO_OUTLINE", "PODCAST_TRANSCRIPT_TURN"):
            slow_pass.results.append(
                ProbeResult(
                    probe=probe, model_id="m:3", model_name="m3", provider="p",
                    success=True, elapsed_s=30.0,
                )
            )

        fast_fail = ModelReport(
            model_id="m:4", model_name="m4", provider="p", model_type="language",
        )
        for probe in ("NOTEBOOK_CHAT", "STUDIO_OUTLINE", "PODCAST_TRANSCRIPT_TURN"):
            fast_fail.results.append(
                ProbeResult(
                    probe=probe, model_id="m:4", model_name="m4", provider="p",
                    success=False, elapsed_s=0.2,
                )
            )
    ...
    def test_partial_pass_ranks_between(self):
        ...
        two_of_three = ModelReport(
            model_id="m:5", model_name="m5", provider="p", model_type="language",
        )
        two_of_three.results = [
            ProbeResult(
                probe="A", model_id="m:5", model_name="m5", provider="p",
                success=True, elapsed_s=10.0,
            ),
            ProbeResult(
                probe="B", model_id="m:5", model_name="m5", provider="p",
                success=True, elapsed_s=10.0,
            ),
            ProbeResult(
                probe="C", model_id="m:5", model_name="m5", provider="p",
                success=False, elapsed_s=10.0,
            ),
        ]
        one_of_three = ModelReport(
            model_id="m:6", model_name="m6", provider="p", model_type="language",
        )
        one_of_three.results = [
            ProbeResult(
                probe="A", model_id="m:6", model_name="m6", provider="p",
                success=True, elapsed_s=5.0,
            ),
            ProbeResult(
                probe="B", model_id="m:6", model_name="m6", provider="p",
                success=False, elapsed_s=5.0,
            ),
            ProbeResult(
                probe="C", model_id="m:6", model_name="m6", provider="p",
                success=False, elapsed_s=5.0,
            ),
        ]
...
class TestJSONExtraction:
    ...
    async def test_clean_json_passes(self):
        ...
        res = await _run_studio_outline_probe(mock_client, {
            "id": "m:1", "name": "m1", "provider": "p",
        })
...
class TestPodcastSpeakerDetection:
    ...
    async def test_both_speakers_passes(self):
        ...
        res = await _run_podcast_turn_probe(mock_client, {
            "id": "m:1", "name": "m1", "provider": "p",
        })
        assert res.success
    ...
    def test_renders_with_three_models(self):
        ...
        for i in range(3):
            r = ModelReport(
                ...
                provider="prov", model_type="language",
            )
            for probe in ("NOTEBOOK_CHAT", "STUDIO_OUTLINE", "PODCAST_TRANSCRIPT_TURN"):
                r.results.append(
                    ProbeResult(
                        probe=probe, model_id=r.model_id, model_name=r.model_name,
                        provider=r.provider,
                        success=(i % 2 == 0),
                        elapsed_s=1.0 + i * 0.5,
                    )
