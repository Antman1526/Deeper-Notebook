...
SCRIPT = (
    Path(__file__).resolve().parents[1] / "scripts" / "live_source_ingestion_smoke.py"
)
...
def test_source_readiness_requires_marker_text_embedding_and_quality():
    ...
    assert (
        smoke.source_is_ready({**ready, "full_text": "hello"}, "onp-smoke-123") is False
    )
    assert smoke.source_is_ready({**ready, "embedded": False}, "onp-smoke-123") is False
    assert (
        smoke.source_is_ready(
            {**ready, "extraction_quality": "no_text"}, "onp-smoke-123"
        )
        is False
    )
