...
Verifies:
  * Default behavior (ONP_ENCRYPTION_KDF unset) is unchanged from
...
def test_migration_sha256_to_pbkdf2_keeps_existing_data_decryptable(
    ...
    (default), encrypts a value, then changes ONP_ENCRYPTION_KDF=pbkdf2
    ...
    # Stage 2: user sets ONP_ENCRYPTION_KDF=pbkdf2 and restarts
    monkeypatch.setenv("ONP_ENCRYPTION_KDF", "pbkdf2")
    enc._reset_encryption_cache()
...
def test_migration_pbkdf2_to_sha256_also_works(monkeypatch):
    ...
    monkeypatch.setenv("ONP_ENCRYPTION_KDF", "sha256")
    enc._reset_encryption_cache()
...
def test_rotation_works_alongside_kdf_migration(monkeypatch):
    ...
    monkeypatch.delenv("ONP_ENCRYPTION_KEYS", raising=False)
    monkeypatch.delenv("ONP_ENCRYPTION_KDF", raising=False)
    enc._reset_encryption_cache()
