...
def test_verifier_rejects_existing_output_and_keeps_aggregate_status_blocked(
    tmp_path: Path,
) -> None:
    ...
    result = run_verifier(
        api_url="http://127.0.0.1:9",
        fixture_root=tmp_path / "fresh",
        output_path=tmp_path / "fresh-proof.json",
    )
    assert result.report["status"] == "blocked"
    assert result.report["synthetic_passed"] is True


def test_source_hash_baseline_precedes_every_proof_action(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "fixture"

    def mutate_after_baseline(_: str) -> tuple[bool, None]:
        (root / "obsidian" / "Pages" / "Plan.md").write_text(
...
def test_default_cli_uses_fresh_owned_child_fixture(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_default_cli_accepts_a_real_temporary_directory(
