...
def test_default_extracts_every_turn(monkeypatch):
    ...
    for i in range(3):
        writer_mod.extract_turn(
            llm=llm, mem_client=mem, chat_session_id="s1",
...
def test_buffers_isolated_per_session(monkeypatch):
    ...
    writer_mod.extract_turn(llm=llm, mem_client=mem, chat_session_id="B",
                            user_text="b0", assistant_text="y")
