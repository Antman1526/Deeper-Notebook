...
def test_role_routing_prefers_specialized_models_without_using_embeddings():
    routes = recommend_model_roles([
        ...
        _model("mlx-community/North-Mini-Code-1.0-6bit", runtime="mlx", params=7, context=32768),
...
def test_role_routing_endpoint_attaches_manifest_matches(app, monkeypatch, tmp_path):
    ...
    roles = {route["role"]: route for route in body["routes"]}
    assert roles["chat"]["manifest_matches"][0]["category"] == "Coding Assistant - Mac MLX"
    assert roles["chat"]["manifest_alignment"]["status"] == "primary"
    assert roles["chat"]["manifest_alignment"]["matched_count"] == 1


def test_role_routing_endpoint_reports_unmatched_manifest_entries(app, monkeypatch, tmp_path):
    ...
    assert manifest_summary["unmatched_entries"][0]["repo"] == "missing/Curated-Model-4bit"


def test_role_routing_endpoint_returns_manifest_reconciliation(app, monkeypatch, tmp_path):
    ...
    by_repo = {
        row["repo"]: row
        for row in manifest_summary["reconciliation_entries"]
    }
...
def test_role_routing_endpoint_reports_manifest_alignment_counts(app, monkeypatch, tmp_path):
    ...
    assert "not in the curated AI_Models manifest" in (
        roles["study_fast"]["manifest_alignment"]["reason"]
    )
    assert roles["study_fast"]["manifest_alternatives"][0]["repo"] == "unsloth/Qwen3-8B-GGUF"
    assert roles["study_fast"]["manifest_alternatives"][0]["matched_model_name"] == "Qwen3-8B-Q4_K_M"
