...
def test_opencode_path_detection(monkeypatch):
    ...
    monkeypatch.setattr("os.path.exists", lambda path: True if path == "/mock/bin/opencode" else False)
...
async def test_run_opencode_success(monkeypatch):
    ...
    out = await run_opencode("print 'Hello'", project="/mock/proj", model="openai/gpt-4o", continue_session=True)
    assert out == "Hello Output"
    
    # Check that it passed the correct arguments
    mock_exec.assert_called_once_with(
        "/mock/bin/opencode", "run", "print 'Hello'",
        "--model", "openai/gpt-4o",
        "--continue",
...
async def test_add_web_source_tool_execution(monkeypatch):
    ...
    assert captures[0]["args"] == {"url": "https://example.com/test", "title": "Custom Title"}
...
async def test_loop_binds_add_web_source_when_notebook_id_present(monkeypatch):
    ...
    assert model.bound is not None
    assert any(getattr(t, "name", None) == "add_web_source_to_notebook" for t in model.bound)
