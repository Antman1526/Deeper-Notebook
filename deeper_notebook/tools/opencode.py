...
async def run_opencode(
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            logger.warning(f"opencode failed with exit code {process.returncode}: {stderr_str}")
...
class OpenCodeRunInput(BaseModel):
    prompt: str = Field(..., description="The prompt/instruction to send to OpenCode to run/execute code or perform a task.")
    project: Optional[str] = Field(None, description="Absolute path to the project directory (defaults to current working directory).")
    model: Optional[str] = Field(None, description="Model to use in provider/model format (e.g., 'openai/gpt-4o').")
    continueSession: Optional[bool] = Field(None, description="Continue the last session instead of starting a new one.")
