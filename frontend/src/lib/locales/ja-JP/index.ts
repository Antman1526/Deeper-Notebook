export const jaJP = {
  ...
  settings: {
    ...
    filesHelp: "ファイルがアップロードされて処理されると、ファイル自体は不要になります。ほとんどのユーザーはOpen notebook+がアップロードフォルダから自動的にファイルを削除することを許可すべきです。",
    ...
    launcherPrefs: {
      ...
      draftModelPathDesc: "llama.cpp の投機的デコーディング用小型 GGUF の絶対パス（OPEN_NOTEBOOK_LOCAL_DRAFT_MODEL_PATH）。無効にするには空白のままにしてください。",
      draftNPredictLabel: "検証ごとのドラフトトークン数",
      draftNPredictPlaceholder: "8",
      draftNPredictDesc: "OPEN_NOTEBOOK_LOCAL_DRAFT_N_PREDICT。デフォルト 8。",
      nCtxLabel: "ローカルコンテキストウィンドウ（n_ctx）",
      nCtxPlaceholder: "32768",
      nCtxDesc: "ONP_CHAT_LLM_CTX。GGUF から自動検出したコンテキスト長を上書きします。空白の場合は自動検出。",
      nCtxMaxLabel: "コンテキストウィンドウの上限（n_ctx_max）",
      nCtxMaxPlaceholder: "32768",
      nCtxMaxDesc: "ONP_CHAT_LLM_CTX_MAX。自動検出コンテキスト長の上限。デフォルト 32768。",
      saveButton: "設定を保存",
  ...
  advanced: {
    ...
    updateAvailableDesc: "Open notebook+の新しいバージョンが利用可能です。",
    upToDate: "最新版です",
    unknown: "不明",
  ...
  models: {
    ...
    defaultAssignmentsDesc: "Open notebook+全体で異なる目的に使用するモデルを設定",
    missingRequiredModels: "必須モデルがありません: {models}。これらがないとOpen notebook+が正しく機能しない可能性があります。",
  ...
  apiKeys: {
    ...
    description: "APIキーをデータベースに安全に保存し、Open notebook+でAIプロバイダーを有効にします。",
  ...
  setupBanner: {
    ...
    goToSettings: "設定へ移動",
    viewDocs: "ドキュメントを見る",
    degraded: "Open notebook+ は劣化状態で動作しています",
