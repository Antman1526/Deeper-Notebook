...
export function useVaultGraph(vaultId?: string, noteId?: string, enabled = true) { return useQuery({ queryKey: vaultKeys.graph(vaultId ?? ''), queryFn: () => vaultApi.graph(vaultId!, noteId!), enabled: Boolean(vaultId && noteId && enabled) }) }
