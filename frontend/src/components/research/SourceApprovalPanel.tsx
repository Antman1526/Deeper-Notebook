...
export function SourceApprovalPanel({ candidates, disabled, onApprove }: { candidates: ResearchCandidate[]; disabled?: boolean; onApprove: (accepted: string[]) => void }) {
  ...
  return (
    <section aria-label="Source approval" className="rounded-md border bg-background p-4">
      ...
      <div className="mt-3 space-y-2">{pending.map((candidate) => <label key={candidate.candidate_id} className="flex cursor-pointer gap-3 rounded-md border p-3 text-sm"><input type="checkbox" checked={accepted.includes(candidate.candidate_id)} onChange={() => toggle(candidate.candidate_id)} /><span className="min-w-0"><span className="block font-medium">{candidate.title ?? candidate.domain}</span><span className="block truncate text-xs text-muted-foreground">{candidate.domain} · {candidate.url}</span>{candidate.snippet ? <span className="mt-1 block text-xs text-muted-foreground">{candidate.snippet}</span> : null}</span></label>)}</div>
