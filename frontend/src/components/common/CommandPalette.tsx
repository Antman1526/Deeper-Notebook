...
} from "@/lib/hooks/use-knowledge-command-data";
...
export function CommandPalette() {
  ...
  const [invocationMode, setInvocationMode] = useState<"global" | "slash">(
    "global",
  );
  ...
  const buildKnowledgeContext =
    useCallback((): KnowledgeCommandExecutionContext | null => {
      const page = useKnowledgeCommandContextStore.getState();
      if (page.generation !== pageContext.generation || !page.context)
        return null;
      ...
      return {
        ...
        setViewMode: (mode) => {
          ...
          if (currentPane && tabId)
            current.setTabViewMode(currentPane.id, tabId, mode);
        },
        splitPane: (direction) => {
          const current = useKnowledgeWorkspaceStore.getState();
          if (current.panes[current.activePaneId])
            current.splitPane(current.activePaneId, direction);
        },
        closePane: () => {
          const current = useKnowledgeWorkspaceStore.getState();
        ...
        moveTab: (offset) => {
          ...
          const currentPane = current.panes[current.activePaneId];
          const activeIndex =
            currentPane?.tabs.findIndex(
              (tab) => tab.id === currentPane.activeTabId,
            ) ?? -1;
          if (!currentPane?.tabs.length || activeIndex < 0) return;
          const target =
            currentPane.tabs[
              (activeIndex + offset + currentPane.tabs.length) %
                currentPane.tabs.length
            ];
  ...
  const commandFilterQuery =
    invocationMode === "slash" ? query.replace(/^\/+/, "") : query;
  const hasCommandMatch =
    ...
    (navigationItems.some(
      ...
      (notebooks?.some(
        ...
        false));
  const showSearchFirst =
    invocationMode === "global" && query.trim() && !hasCommandMatch;
  ...
  const semanticResults = useMemo(
    () =>
      invocationMode === "global" &&
      open &&
      indexed.semantic.variables === query.trim()
        ? indexed.semantic.data?.results || []
        : [],
    [
      indexed.semantic.data?.results,
      indexed.semantic.variables,
      invocationMode,
      open,
      query,
    ],
  ...
  const executeKnowledge = useCallback(
    async (id: Parameters<typeof executeKnowledgeCommand>[0]) => {
  ...
  const selectTab = useCallback(
    (tab: ReturnType<typeof searchResultToOpenTab>) => {
      if (!tab) return;
    ...
    >
      <div className="relative">
        {invocationMode === "slash" && (
          <span
            aria-hidden="true"
            className="pointer-events-none absolute left-10 top-3 z-10 text-sm"
          >
            /
          </span>
        )}
        <CommandInput
          ...
          onValueChange={(value) =>
            setQuery(invocationMode === "slash" ? `/${value}` : value)
          }
        ...
        {invocationMode === "global" && (
          <>
                ...
                {notebooksLoading ? (
                  ...
                  notebooks!.map((notebook) => (
                    <CommandItem
                      ...
                      onSelect={() =>
                        handleNavigate(`/notebooks/${notebook.id}`)
                      }
                    >
            ...
            {exactCandidates.length > 0 && (
              <CommandGroup heading={t("knowledge.exactResults")}>
                {exactCandidates.map((candidate) => (
                  <CommandItem
                    key={candidate.key}
                    value={`exact ${candidate.title} ${candidate.relativePath}`}
                    onSelect={() => selectTab(candidateToOpenTab(candidate))}
                  >
              ...
              <CommandGroup heading={t("knowledge.indexedSearchResults")}>
                {indexedResults.map((result) => {
                  ...
                  return (
                    <CommandItem
                      key={result.id}
                      value={`indexed ${tab.title} ${tab.relativePath}`}
            ...
            {embeddingConfigurationError(indexed.semantic.error) && (
              <CommandGroup
                heading={t("knowledge.semanticUnavailable")}
                forceMount
              >
                <CommandItem
                  value="configure embedding model"
                  onSelect={() => handleNavigate("/settings/api-keys")}
                  forceMount
                >
                  <Settings className="h-4 w-4" />
                  <span>{t("knowledge.semanticUnavailable")}</span>
                </CommandItem>
              </CommandGroup>
            )}
                ...
                {semanticResults.map((result) => {
                  ...
                  if (!tab)
                    return (
                      <CommandItem
                        key={result.id}
                        value={`semantic search ${result.title}`}
                        onSelect={handleSearch}
                      >
                        <Search className="h-4 w-4" />
                        <span>{result.title}</span>
                  ...
                  return (
                    <CommandItem
                      key={result.id}
                      value={`semantic result ${tab.title} ${tab.relativePath}`}
                      onSelect={() => selectTab(tab)}
                    >
                      <FileText className="h-4 w-4" />
                      <span>{tab.title}</span>
              ...
              <>
                ...
                <CommandGroup heading={t("searchPage.orSearchKb")} forceMount>
                  ...
                  >
                    <Search className="h-4 w-4" />
                    <span>
                      {t("searchPage.searchResultsFor").replace(
                        "{query}",
                        query,
                      )}
                    </span>
                  ...
                  >
                    <MessageCircleQuestion className="h-4 w-4" />
                    <span>
                      {t("searchPage.askAbout").replace("{query}", query)}
                    </span>
                  </CommandItem>
                </CommandGroup>
              </>
