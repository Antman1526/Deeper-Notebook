...
export function LocalExecutionPolicyPanel({ policy, computeProfile, memoryLimitBytes, pendingCloudRoute = null, onConfirmCloudRoute, isSaving, onSave }: Props) {
  ...
  return <Card data-testid="local-execution-policy">
    ...
    <CardContent className="space-y-4">
      ...
      <Button className="h-auto min-h-11 w-full whitespace-normal sm:w-auto" type="button" disabled={Boolean(isSaving) || !validLimit} onClick={() => onSave({ execution_policy: nextPolicy, compute_profile: nextProfile, local_model_memory_limit_bytes: limit })}>Save local execution policy</Button>
