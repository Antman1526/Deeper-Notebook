...
function matchFixturePageRoute(pathname: string): {
  ...
  >) {
    for (const pagePath of pagePaths) {
      ...
      if (pathname === `${pagePath}/outgoing`) return { name, route: 'outgoing' }
      if (pathname === `${pagePath}/backlinks`) return { name, route: 'backlinks' }
    }
  }
  return null
}
...
export async function fulfillKnowledgeRequest(
  ...
  if (path.endsWith('/deeper-notebook/workspace/knowledge')) {
    ...
    payload = pageRoute.route === 'outgoing'
    ...
    payload = {
      operation_id: 'scan:fixture', state: 'completed', observed: 2, parsed: 2,
      unchanged: 0, unsupported: 0, invalid: 0, missing: 0, embeddings_pending: 0,
    }
  } else if (
...
export async function fulfillKnowledgeSearchRequest(
  ...
  if (body.type === 'vector' && !state.embeddingAvailable) {
    await route.fulfill({
      status: 400,
      contentType: 'application/json',
      body: JSON.stringify({ detail: 'Vector search requires an embedding model' }),
    })
    return
  }
  await route.fulfill({
    ...
    body: JSON.stringify({
      results: [{
        id: 'note:plan', title: 'Plan', parent_id: 'vault:fixture', final_score: 1,
        created: '', updated: '',
        vault_provenance: {
          canonical_external: true, vault_id: 'vault:fixture',
          relative_path: 'pages/plan.md', source_hash: planFile.content_hash,
        },
      }],
      total_count: 1,
