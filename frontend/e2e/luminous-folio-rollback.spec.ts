...
test('legacy Studio keeps the shell main landmark singular', async ({ page, researchWorkbench }) => {
  test.skip(
    process.env.NEXT_PUBLIC_DN_LUMINOUS_FOLIO !== '0',
    'The rollback proof requires a build created with NEXT_PUBLIC_DN_LUMINOUS_FOLIO=0.',
  )
  ...
  await expect(page.locator('main')).toHaveCount(1)
  await expect(page.locator('main[aria-label="Evidence Studio folio"]')).toHaveCount(0)
  await expect(page.locator('section[aria-label="Evidence Studio folio"]')).toBeVisible()
})
