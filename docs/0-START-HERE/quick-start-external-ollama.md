...
Run Deeper Notebook with a **separately installed Ollama** (not via Docker). This avoids Docker running the Ollama service while you use your own local Ollama installation.

## Prerequisites

1. **Docker Desktop** installed (for SurrealDB and Deeper Notebook)
...
## Step 3: Connect Deeper Notebook to Host Ollama (1 min)

When Deeper Notebook runs inside Docker, it cannot reach `localhost:11434` on your host directly. Use the special hostname:

| Host OS | Ollama URL in Deeper Notebook |
|---------|----------------------------|
...
## Step 4: Start Deeper Notebook (1 min)
...
## Step 7: Access Deeper Notebook (instant)
...
- **Add more models**: Run `ollama pull <model>`, then re-discover from Deeper Notebook
