...
Use any server that implements the OpenAI API format with Deeper Notebook. This includes LM Studio, Text Generation WebUI, vLLM, and many others.
...
Deeper Notebook can connect to any server using this format.
...
### Step 3: Add Model in Deeper Notebook
...
When Deeper Notebook runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Settings → API Keys**:
...
## Adding Models in Deeper Notebook
...
4. Update model name in Deeper Notebook
```
